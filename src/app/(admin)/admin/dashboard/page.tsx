...
export default function DashboardPage() {
  ...
  const loadData = async (tid: string) => {
    ...
    const { data: orders } = await supabase.from('orders')
      ...
      .eq('tenant_id', tid).eq('is_test', false).gte('created_at', todayStart.toISOString()).order('created_at', { ascending: false });
    if (orders) {
      const completed = orders.filter(o => o.status === 'COMPLETED');
      const totalSales = completed.reduce((s, o) => s + Number(o.total_amount), 0);
      setStats({ todaySales: totalSales, todayOrders: orders.length, pendingOrders: orders.filter(o => ['PENDING','CONFIRMED','PREPARING'].includes(o.status as string)).length, completedOrders: completed.length, cancelledOrders: orders.filter(o => o.status === 'CANCELLED').length, avgOrderValue: completed.length > 0 ? totalSales / completed.length : 0 });
  ...
  useEffect(() => {
    let tid = '';
    try { tid = (JSON.parse(localStorage.getItem('tyg_session') ?? '{}') as { tenantId?: string }).tenantId ?? ''; } catch { /* */ }
    if (!tid) try { tid = (JSON.parse(localStorage.getItem('tyg_tenant') ?? '{}') as { id?: string }).id ?? ''; } catch { /* */ }
    if (tid) { setTenantId(tid); void loadData(tid); }
    else setLoading(false);
  }, []);

  useEffect(() => {
    if (!tenantId) return;
    const ch = supabase.channel('dash').on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `tenant_id=eq.${tenantId}` }, () => void loadData(tenantId)).subscribe();
    ...
    <div style={{ color: C.text, fontFamily: "'Sora','Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={() => { if (tenantId) { setRefreshing(true); void loadData(tenantId); } }}
          ...
          <div style={{ padding: 12 }}>
            {loading ? (
              ...
              <div key={order.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px', borderRadius: 12, marginBottom: 6, background: C.surface2 }}>
                <div style={{ width: 4, height: 36, borderRadius: 99, background: STATUS_COLOR[order.status], flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                    <span style={{ fontWeight: 700, fontSize: 14 }}>#{order.order_number}</span>
                    <span style={{ padding: '2px 8px', borderRadius: 99, fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', background: STATUS_BG[order.status], color: STATUS_COLOR[order.status] }}>
                      {order.status}
                    </span>
                  </div>
