...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const [categories, items] = await Promise.all([
      ...
      pgGet<RawItem>(`menu_items?${itemFilter}&select=id,category_id,name,description,image_url,base_price,status,sort_order,is_featured,tags,stock_count,low_stock_threshold,has_sugar_level`),
    ]);
