...
const CreateItemSchema = z.object({
  ...
  status: z.enum(['AVAILABLE','UNAVAILABLE','HIDDEN']).optional().default('AVAILABLE'),
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  return withStaffAuth(req, async (request, ctx) => {
    ...
    const { data: cat } = await db.from('menu_categories').select('id').eq('id', parsed.data.categoryId).eq('tenant_id', ctx.tenantId).single();
    ...
    if (error) return apiError('Failed to create item', 500);
