...
export default function TablesPage() {
  ...
  useEffect(() => {
    try {
      const t = JSON.parse(localStorage.getItem('tyg_tenant') || '{}') as { name?: string; slug?: string; address?: string };
      setSession(s => ({ ...s, tenantName: t.name ?? s.tenantName, tenantSlug: t.slug ?? s.tenantSlug, tenantAddress: t.address ?? s.tenantAddress }) as SessionData);
    } catch {/**/}
    try {
      const s = JSON.parse(localStorage.getItem('tyg_session') || '{}') as SessionData;
      setSession(p => ({ ...p, ...s }));
    } catch {/**/}
  }, []);
  ...
  const copyUrl = async (t: Table) => {
    try {
      await navigator.clipboard.writeText(orderUrl(t));
      setCopied(t.id);
      setTimeout(() => setCopied(null), 2000);
    } catch {
      showToast('Copy failed — select the link manually', 'err');
    }
  };
  ...
  return (
    <div style={{ color: 'var(--text)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>
            {tables.length} tables — copy each link to create your own QR code
          </p>
        </div>
      ...
      <div style={{ marginBottom: 20, padding: '14px 18px', borderRadius: 12, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.2)', fontSize: 13, color: 'var(--text-muted)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        <div>
          <strong style={{ color: 'var(--text)' }}>How to make QR codes:</strong>
          <span> Copy a link below → paste into </span>
          <a href="https://www.qrcode-monkey.com" target="_blank" rel="noreferrer" style={{ color: '#6366f1' }}>QRCode Monkey</a>
          <span> or Canva → download → print.</span>
        </div>
      </div>
