        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%,280px), 1fr))', gap: 16 }}>
          {tables.map(t => {
            ...
            <div key={t.id} style={{ ...card, overflow: 'hidden', outline: tableOrder ? `2px solid ${statusCfg?.color ?? '#22c55e'}` : 'none' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'white' }}>
              ...
              <div style={{ padding: 16 }}>
                ...
                <a href={orderUrl(t)} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 0', borderRadius: 10, fontSize: 13, fontWeight: 600, background: 'var(--surface-2)', color: 'var(--text-muted)', border: '1px solid var(--border)', textDecoration: 'none' }}><QrCode size={14} /> Preview Order Page</a>
              </div>
            </div>
            );
          })}
